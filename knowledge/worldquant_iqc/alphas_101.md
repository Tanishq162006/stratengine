# 101 Formulaic Alphas — Reference List

Source: Kakushadze, "101 Formulaic Alphas" (Wilmott Magazine).

The WQ Price-Volume seminar recommends improving 6–8 of these for
submission. Aggregate stats over the 2010–2013 test:

- **Sharpe** (min / Q1 / median / Q3 / max): 1.2 / 1.9 / 2.2 / 2.5 / 4.2
- **Turnover**: 0.16 / 0.34 / 0.48 / 0.65 / 1.6
- **Cents-per-share**: 0.13 / 0.31 / 0.40 / 0.51 / 2.0
- Pairwise correlation across the 101: ~15.9%.
- Returns vs volatility: `returns ~ volatility ^ 0.76`.

Mean-reversion alphas tend to outperform in 2020–2021 regimes.
**Alphas 33 and 101 are simple and especially strong.** Alpha 57 is also
known to be tunable to a submittable form.

The list below preserves the original syntax (`Ts_ArgMax`, `IndNeutralize`,
`IndClass.subindustry`, `decay_linear`, etc.) — these map onto the BRAIN
operators we already document. A few translations to keep handy:
...
| `IndClass.{market, sector, industry, subindustry}` | bare `market`, `sector`, `industry`, `subindustry` |
| `decay_linear(...)` | `ts_decay_linear` |
| `delta(x, d)` / `delay(x, d)` | `ts_delta(x, d)` / `ts_delay(x, d)` |
| `correlation(x, y, d)` | `ts_corr(x, y, d)` |
| `covariance(x, y, d)` | `ts_cov(x, y, d)` |
| `stddev(x, d)` | `ts_std_dev(x, d)` |
| `sum(x, d)` | `ts_sum(x, d)` |
| `product(x, d)` | `ts_product(x, d)` |
| `signedpower(x, n)` / `SignedPower` | `signed_power(x, n)` |
| `Sign(x)` | `sign(x)` |
| `^` | `pow(x, n)` (note: BRAIN may also accept `^` directly) |

When using the 101 list in synth, **first translate to BRAIN syntax**, then
wrap the result in the canonical `scale(group_neutralize(...))` pipeline if
the source did not.
...
05. `(rank((open - (sum(vwap, 10) / 10))) * (-1 * abs(rank((close - vwap)))))`
06. `(-1 * correlation(open, volume, 10))`
07. `(adv20 < volume) ? ((-1 * ts_rank(abs(delta(close, 7)), 60)) * sign(delta(close, 7))) : (-1 * 1)`
...
- Adv-N variants (`adv15`, `adv30`, `adv40`, `adv50`, `adv60`, `adv81`,
  `adv120`, `adv150`, `adv180`) are all valid `advN` BRAIN fields.
- Many of these output a 0/1 boolean comparison (`* -1`). Wrap with
  `scale(group_neutralize(zscore(...), subindustry))` before submitting.
- Short `ts_decay_linear` windows + `rank` + relative values (pcts) help
  performance per the seminar notes.
